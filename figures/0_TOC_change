...
# ----------------------------
# Compute replicate-level TOC change (TF - T0)
# ----------------------------
toc_df = df[df["DOC/TOC"] == "TOC"].copy()
...
rep_tbl["TOC_change"] = rep_tbl["TF"] - rep_tbl["T0"]

# Mean + SD per Experiment × Water_Treatment
summ = (
    rep_tbl
    .groupby(["Experiment", "Water_Treatment"], as_index=False)["TOC_change"]
    .agg(mean="mean", sd="std")
)

# ----------------------------
# Parse treatment labels + colors (same scheme as your other figures)
# ----------------------------
...
plt.ylabel("Change in TOC (TF - T0, µMC)")
plt.tight_layout()

png_path = "/mnt/data/TOC_change_highres.png"
...
# ----------------------------
pdf_path = "/mnt/data/TOC_change_high_quality.pdf"
